...
# The standard functional fixtures are registered as a plugin by the top-level
# tests/conftest.py. pytest rejects pytest_plugins in a non-top-level conftest,
...
def unique_schema(request, prefix) -> str:
    ...
    if provided_test_schema:
        ...
        custom_suffix = (
            "_" + os.getenv("DBT_GLUE_CUSTOM_SCHEMA_SUFFIX")
            if os.getenv("DBT_GLUE_CUSTOM_SCHEMA_SUFFIX")
            else ""
        )
